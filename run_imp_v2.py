...
import numpy as np
import random
from trainer_imp_v2 import Trainer
import re
...
def get_args(argv):
    ...
    # --- START OF FIX ---
    # Manually map keys from the YAML structure to what the trainer expects
    
    # 1. Map Dataset keys
    if 'name' in dataset_config:
        final_config['dataset'] = dataset_config['name']
    final_config.update(dataset_config) # Add the rest (dataroot, workers, etc.)
    ...
    # --- V2: Override learner type and name ---
    final_config['learner_type'] = 'apt_imp_v2'
    final_config['learner_name'] = 'APT_IMP_V2_Learner'
    
    # Set log_dir based on convention
    if args.log_dir is None:
        dataset_name = final_config.get('dataset', 'cifar100').lower()
        final_config['log_dir'] = f'./checkpoints/{dataset_name}-imp-v2/seed{args.seed}'
...
if __name__ == '__main__':
    ...
    start_r = 0
    # --- V2: Run only one repeat based on seed ---
