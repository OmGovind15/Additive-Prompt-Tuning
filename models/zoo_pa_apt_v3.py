...
from .vit_pa_apt_v2 import VisionTransformer
...
class APT(nn.Module):
    def __init__(self, emb_d, n_tasks, prompt_param, ema_coeff, patch_selector_fn=None): # ADDED learner_instance
        ...
        # --- V2 MODIFICATION: Store the learner ---
        self.patch_selector_fn = patch_selector_fn # This line fixes the error
        # --- END MODIFICATION ---
    ...
    def forward(self, l, x_block, train=False):
        ...
        # MODIFIED: Add the learner instance (patch selector) to the prompt list
        P = [P_k, P_v, self.patch_selector_fn]
...
class ViTZoo(nn.Module):
    def __init__(self, num_classes=10, ema_coeff=0.5, pt=False, prompt_flag=False, prompt_param=None, tasks=[], k_patches=5, use_layer_wise=False, patch_selector_fn=None):
            ...
            if self.prompt_flag == 'apt':
                # MODIFIED: Pass learner_instance to APT
                self.prompt = APT(768, prompt_param[0], prompt_param[1], ema_coeff=ema_coeff,patch_selector_fn=patch_selector_fn)
            else:
                self.prompt = None
    
            # Find all prompt parameters (CLS and Patch)
            prompt_params = []
            for name, param in self.feat.named_parameters():
                if 'patch_p' in name: # patch_p_k and patch_p_v
                    prompt_params.append('feat.' + name) # Need 'feat.' prefix
            ...
            tuned_params = [
                ...
                "last.weight",
                "last.bias", 
            ] + prompt_params # Add patch prompts to tuned params
...
def vit_pt_imnet(out_dim, ema_coeff, tasks=[], prompt_flag = 'None', prompt_param=None, k_patches=5, use_layer_wise=False, patch_selector_fn=None):
    # MODIFIED: Added all V3 parameters here
    return ViTZoo(num_classes=out_dim, ema_coeff=ema_coeff, pt=True, prompt_flag=prompt_flag, prompt_param=prompt_param, tasks=tasks,
                  k_patches=k_patches, use_layer_wise=use_layer_wise, patch_selector_fn=patch_selector_fn)
