...
class APT(nn.Module):
    def __init__(self, emb_d, n_tasks, prompt_param, ema_coeff):
        ...
        # Buffer to store the old prompt for fusion
        old_prompt = torch.zeros(12*2, emb_d).cuda()
        self.register_buffer('old_prompt', old_prompt.clone().detach())

        # Buffer to store the Fisher importance scores
        fisher_importance = torch.zeros(12*2, emb_d).cuda()
        self.register_buffer('fisher_importance', fisher_importance.clone().detach())
    ...
    def merge_prompt(self, new_prompt):
        print("Merging prompt with importance-based fusion...")

        # Get the old prompt and its importance scores
        old_prompt = self.old_prompt
        fisher = self.fisher_importance

        # Normalize importance scores (e.g., to 0-1 range)
        # This helps in creating a stable weight
        min_val = fisher.min()
        max_val = fisher.max()
        if max_val - min_val > 1e-5: # Avoid division by zero
            normalized_fisher = (fisher - min_val) / (max_val - min_val)
        else:
            normalized_fisher = torch.zeros_like(fisher)

        # Create the fusion weights (alpha)
        # This is the core logic:
        # If importance is HIGH (normalized_fisher -> 1), alpha -> self.ema_coeff (e.g., 0.7)
        # If importance is LOW (normalized_fisher -> 0), alpha -> (1 - self.ema_coeff) (e.g., 0.3)
        # This way, important params keep more of the OLD value

        # We adjust the base ema_coeff to control the range
        base_alpha = self.ema_coeff # e.g., 0.7
        min_alpha = 1.0 - base_alpha # e.g., 0.3

        # Calculate the dynamic alpha for each parameter
        alpha = min_alpha + (base_alpha - min_alpha) * normalized_fisher

        # Perform the weighted fusion
        merged_prompt = (alpha * old_prompt) + ((1.0 - alpha) * new_prompt)

        # Save the new prompt as the "old" prompt for the NEXT task
        self.old_prompt.copy_(merged_prompt)

        # Update the global merged prompt for inference
        self.global_merged_prompt.copy_(merged_prompt)
