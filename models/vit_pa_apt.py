...
class Attention(nn.Module):
    ...
    def forward(self, x, register_hook=False, prompt=None,layer=-1):
        ...
        k_mod = k.clone()
        v_mod = v.clone()
        # --- START PA-APT MODIFICATION ---

        # 1. PROBE PASS: Compute attention scores to find top-k patches
        attn_scores_probe = (q @ k.transpose(-2, -1)) * self.scale
        attn_softmax_probe = attn_scores_probe.softmax(dim=-1) # Shape: [B, H, N, N]
        
        # 2. SELECT TOP-K: Get CLS-to-patch scores
        # We average over heads (dim=1) to get one score per patch
        # We look at the 0-th query (CLS)
        # We skip the 0-th key (CLS-to-CLS score)
        cls_to_patch_scores = attn_softmax_probe.mean(dim=1)[:, 0, 1:] # Shape: [B, N-1]
        
        # Get the indices of the top-k patches
        # topk_indices Shape: [B, k]
        topk_indices = torch.topk(cls_to_patch_scores, k=self.k, dim=-1)[1]
        
        # Add 1 to indices to account for skipping the CLS token
        # Indices are now in the range [1, N-1]
        ...
        if prompt is not None:
            if type(prompt) is list and len(prompt) == 2:
                ...
                # 3a. Apply original CLS prompts (index 0)
                k_mod[:,:,0:1] = k_mod[:,:,0:1] + pk[:,:,0:1]
                v_mod[:,:,0:1] = v_mod[:,:,0:1] + pv[:,:,0:1]
                
                # 3b. Apply new Patch Prompts using scatter_add_
                B_h, H, N_k, D_h = k.shape
                
                # Expand indices to match K/V shape for scatter_add
                # Shape: [B, H, k, D_h]
                idx_expanded = topk_indices.unsqueeze(1).unsqueeze(-1).expand(B_h, H, self.k, D_h)
                
                # Expand patch prompts to be the source
                # Shape: [B, H, k, D_h]
                patch_pk_src = self.patch_p_k.expand(B_h, H, self.k, D_h)
                patch_pv_src = self.patch_p_v.expand(B_h, H, self.k, D_h)
        ...
        # 4. FINAL PASS: Re-compute attention with *modified* K and V
