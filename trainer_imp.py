...
class Trainer:
    ...
    def __init__(self, args, seed, cur_iter, metric_keys, save_keys):
        ...
        if self.learner_type == 'apt_imp':
            self.learner = learners.apt_imp.__dict__[self.learner_name](self.learner_config)
        else:
    ...
    def train(self, avg_metrics):
        ...
        for i in range(self.max_task):
            ...
            # Calculate and store importance scores
            if i == 0:
            # For the first task, just save the prompt as "old"
                try:
                    self.learner.model.module.prompt.old_prompt.copy_(self.learner.model.module.prompt.prompt_tokens.data)
                except:
                    self.learner.model.prompt.old_prompt.copy_(self.learner.model.prompt.prompt_tokens.data)

            # Calculate importance using the training data loader
            self.learner.calculate_importance(train_loader)
    ...
    def evaluate(self, avg_metrics):
        ...
        for i in range(self.max_task):
            ...
            # --- START TASK 2 MODIFICATION ---
            # Manually merge prompts before evaluation
            if i > 0:
                try:
                    new_prompt = self.learner.model.module.prompt.prompt_tokens.data
                    self.learner.model.module.prompt.merge_prompt(new_prompt)
                except:
                    new_prompt = self.learner.model.prompt.prompt_tokens.data
                    self.learner.model.prompt.merge_prompt(new_prompt)
            # --- END TASK 2 MODIFICATION ---
